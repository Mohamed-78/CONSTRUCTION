@extends('admin.layouts.default',['title' => 'PLANS & CÔUT'])

@section('content')
      <div class="app-title">
        <div>
          <h1 style="font-size: 16px;"><i class="fa fa-dashboard"></i> LISTE DES PLANS DE TERRAINS PAR DIMENSION : {{$dimension->count()}}</h1>
        </div>
      ...
      <div class="row">
    <div class="col-12">
      <a href="/managment/ajouter/dimensionT" class="btn btn-info ajouter_equipe"> <i class="fa fa-edit"></i> Ajouter un plan</a><br><br>
    </div>

        <div class="col-md-12">
          <div class="tile">
            <div class="tile-title-w-btn">
              <h3 class="title">Tous les plans et coût  de terrain par dimension</h3></div>
            <div class="tile-body">
              ...
              @if($dimension)
               <table class="table table-hover table-bordered" id="sampleTable">
                    ...
                    <tbody>
                      @foreach($dimension as $ac)
                        <tr>
                          <td><img src="{{URL::asset('admin/media/'.$ac->photo)}}" alt="" style="max-height: 50px; max-width: 50px;"></td>
                          <td>{{$ac->titre}}</td>
                          <td>{{$ac->prix}}</td>
                          <td>{{ $ac->dimension }}</td>
                          <td>@if(isset($ac->description)){{substr($ac->description,0,45)}}{{ (strlen($ac->description) >= 45) ? '...' : '' }}
                            @else 
                        Aucune description
                        @endif
                          </td>
                          <td valign="center">
                            <div align="center" class="tools">
                              <a href="/managment/update/dimensionT/{{$ac->id}}" class="btn bg-green btn-flat voir_docoment" style="width: 10px;">
                                <i class="fa fa-eye"></i>
                              </a> 
                              
                              <a href="/managment/delete/dimensionT/{{$ac->id}}" class="btn bg-orange btn-flat delete_equipe" style="width: 10px;">
...
<script type="text/javascript">
      ...
      $(document).on("click",".delete_equipe", function(e){
        ...
        swal({
          title: "SUPPRESSION",
          text: "Etre Vous sûre de vouloir supprimer ce plan ?",
          type: "warning",
          ...
          if (isConfirm) {
            swal("Sppression!", "Plan supprimée avec succès .", "success");
